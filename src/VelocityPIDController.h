 ...
 * PIDOutput
 */
class VelocityPIDController : public LiveWindowSendable, public Controller, public ITableListener
{
public:
	VelocityPIDController(float p, float i, float d, PIDSource *source, PIDOutput *output, float period =
			0.05);
	VelocityPIDController(float p, float i, float d, float f, PIDSource *source, PIDOutput *output,
			float period = 0.05);
	virtual ~VelocityPIDController();
	virtual float Get();
...
protected:
	...
	DISALLOW_COPY_AND_ASSIGN(VelocityPIDController);
};
