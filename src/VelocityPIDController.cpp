 ...
 */
VelocityPIDController::VelocityPIDController(float Kp, float Ki, float Kd,
 ...
 * integral and differental terms. The default is 50ms.
 */
VelocityPIDController::VelocityPIDController(float Kp, float Ki, float Kd, float Kf,
								PIDSource *source, PIDOutput *output,
								float period) :
	m_semaphore (0)
{
...
void VelocityPIDController::Initialize(float Kp, float Ki, float Kd, float Kf,
								PIDSource *source, PIDOutput *output,
								float period)
{
	m_table = NULL;
	...
	m_controlLoop = new Notifier(VelocityPIDController::CallCalculate, this);
 ...
 */
void VelocityPIDController::CallCalculate(void *controller)
{
	VelocityPIDController *control = (VelocityPIDController*) controller;
	control->Calculate();
}
  ...
  */
void VelocityPIDController::Calculate()
{
	bool enabled;
	PIDSource* pidInput;
	PIDOutput* pidOutput;
	...
	if (enabled)
	{
		...
		float result; //this is what is written to motor
		PIDOutput* pidOutput;
		...
		pidOutput = m_pidOutput;
		result = m_result;
		//result is what is written to the motor
		pidOutput->PIDWrite(result);
	}
	else
	{
		pidOutput->PIDWrite(m_setpoint);
	}
}
 ...
 */
void VelocityPIDController::SetPID(float p, float i, float d)
{
	CRITICAL_REGION(m_semaphore)
	{
		m_P = p;
 ...
 */
void VelocityPIDController::SetPID(float p, float i, float d, float f)
{
	CRITICAL_REGION(m_semaphore)
	{
		m_P = p;
 ...
 */
float VelocityPIDController::GetP()
{
	CRITICAL_REGION(m_semaphore)
	{
		return m_P;
 ...
 */
float VelocityPIDController::GetI()
{
	CRITICAL_REGION(m_semaphore)
	{
		return m_I;
 ...
 */
float VelocityPIDController::GetD()
{
	CRITICAL_REGION(m_semaphore)
	{
		return m_D;
 ...
 */
float VelocityPIDController::GetF()
{
	CRITICAL_REGION(m_semaphore)
	{
		return m_F;
 ...
 */
float VelocityPIDController::Get()
{
	float result;
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
void VelocityPIDController::SetContinuous(bool continuous)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
void VelocityPIDController::SetInputRange(float minimumInput, float maximumInput)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
void VelocityPIDController::SetOutputRange(float minimumOutput, float maximumOutput)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
void VelocityPIDController::SetSetpoint(float setpoint)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
float VelocityPIDController::GetSetpoint()
{
	float setpoint;
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
float VelocityPIDController::GetError()
{
	float error;
  double pidInput;
	CRITICAL_REGION(m_semaphore)
 ...
 */
void VelocityPIDController::SetTolerance(float percent)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 * @param percentage error which is tolerable
 */
void VelocityPIDController::SetPercentTolerance(float percent)
{
	CRITICAL_REGION(m_semaphore)
	{
		m_toleranceType = kPercentTolerance;
 ...
 * @param percentage error which is tolerable
 */
void VelocityPIDController::SetAbsoluteTolerance(float absTolerance)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
bool VelocityPIDController::OnTarget()
{
	bool temp;
 ...
 */
void VelocityPIDController::Enable()
{
	CRITICAL_REGION(m_semaphore)
	{
		m_enabled = true;
 ...
 */
void VelocityPIDController::Disable()
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
bool VelocityPIDController::IsEnabled()
{
	bool enabled;
	CRITICAL_REGION(m_semaphore)
	{
...
std::string VelocityPIDController::GetSmartDashboardType(){
	return "VelocityPIDController";
}

void VelocityPIDController::InitTable(ITable* table){
	if(m_table!=NULL)
...
ITable* VelocityPIDController::GetTable(){
	return m_table;
}

void VelocityPIDController::ValueChanged(ITable* source, const std::string& key, EntryValue value, bool isNew){
...
void VelocityPIDController::UpdateTable() {

}

void VelocityPIDController::StartLiveWindowMode() {
	Disable();
}

void VelocityPIDController::StopLiveWindowMode() {
